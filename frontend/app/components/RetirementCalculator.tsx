...
import { CURRENCIES, formatFull, getCurrency } from "@/lib/currency";
import type { ResolvedLocationDefaults } from "@/lib/locationDefaults";
  ...
  useEffect(() => {
    async function applyLocationDefaults() {
      try {
        ...
        if (!res.ok) return;
        const data: { countryCode: string; source: string; isSupported: boolean; defaults: ResolvedLocationDefaults } =
          await res.json();

        const { defaults, countryCode } = data;

        // Apply location-specific currency
        const detectedCurrency = getCurrency(defaults.currencyCode);
        setCurrency(detectedCurrency);

        // Apply location-specific financial parameters
        setFormData((prev) => ({
          ...prev,
          annual_return: defaults.annualReturn,
          inflation_rate: defaults.inflationRate,
          current_savings: defaults.currentSavings,
          monthly_contribution: defaults.monthlyContribution,
          current_yearly_spending: defaults.currentYearlySpending,
        }));

        // Keep the annual_return preset in sync
        const matchingPreset = Object.entries(PRESETS).find(
          ([, p]) => p.annual_return === defaults.annualReturn
        );
        setActivePreset(matchingPreset ? matchingPreset[0] : null);

        // Show a brief banner for non-US locales
        if (countryCode !== "US" && data.isSupported) {
          setLocationBanner(`Defaults set for ${countryCode} (${defaults.currencyCode})`);
          setTimeout(() => setLocationBanner(null), 5000);
        }
      } catch {
        // Silently ignore — keep the hardcoded US defaults
      }
    }
  ...
  return (
    <div className="w-full max-w-6xl space-y-6">
      ...
      {locationBanner && (
        ...
        >
          <span>📍 {locationBanner} — you can change currency or any value above.</span>
          <button
